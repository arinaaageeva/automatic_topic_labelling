...
class TopicModel(BaseEstimator, ClusterMixin):
    ...
    def __init__(self, 
        ...
        self.model = artm.hARTM(dictionary=dictionary, class_ids=class_ids, theta_columns_naming=theta_columns_naming, tmp_files_path=tmp_files_path, cache_theta=cache_theta)
        ...
        if parent_topics_proportion is not None:
            ...
            for level, parent_topics_proportion_level in zip(self.model[1:], parent_topics_proportion):
                for topic_name, parent_topic_proportion in parent_topics_proportion_level.items(): 
                    level.regularizers.add(artm.HierarchySparsingThetaRegularizer(name=f'HST_{topic_name}', topic_names=topic_name, 
                                                                                  tau=spars_psi_tau, parent_topic_proportion=parent_topic_proportion))  
            
        for level in self.model:
            for class_id, weight in class_ids.items():
                if weight > 0:
                    level.scores.add(artm.TopTokensScore(name=f'{class_id}_top_tokens', class_id=class_id, num_tokens=num_tokens))
    ...
    def fit_supervised(self, model, X, y):
        ...
        model.regularizers.add(artm.SmoothSparseThetaRegularizer(name='SST', tau=self.smooth_theta_fit, 
                                                                 doc_titles=y.index, doc_topic_coef=doc_topic_coef.tolist()))
    ...
    def validation(self, 
        ...
        for train_indecies, test_indecies in self.kfold.split(X.values, y.values):
            ...
            X_train = artm.BatchVectorizer(data_path=os.path.join(tmp_files_path, 'X_train.txt'), 
                                           data_format='vowpal_wabbit', target_folder=os.path.join(tmp_files_path, 'X_train'))
            X_test = artm.BatchVectorizer(data_path=os.path.join(tmp_files_path, 'X_test.txt'), 
                                          data_format='vowpal_wabbit', target_folder=os.path.join(tmp_files_path, 'X_test'))
            ...
            calculation_score = lambda score, y_pred, y: [score(y[column], y_pred[column]) for column in y.columns]
